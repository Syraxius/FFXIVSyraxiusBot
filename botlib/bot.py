...
class Bot:
    ...
    def walk(self, recording, destination):
        ...
        while True:
            ...
            if next_coordinate is None:
                next_coordinate = shortest_path.pop(0)
            direction_delta, distance_delta, is_turn_left = self.calculate_navigation(next_coordinate[0], next_coordinate[1])
            ...
            if distance_delta < 1:
                ...
                if turn_duration > 0:
                    if is_turn_left:
                        ...
                        keyboard_send_vk_as_scan_code(self.hwnd, win32api.VkKeyScanEx('d', 0), action='hold', duration=turn_duration)  # Hotkey for turn right
                if not is_autorun:
                    keyboard_send_vk_as_scan_code(self.hwnd, win32api.VkKeyScanEx('r', 0))
                    is_autorun = True
            time.sleep(0.05)
